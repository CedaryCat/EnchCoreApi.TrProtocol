                        ...
                        file.BlockWrite((source) => {
                            ...
                            source.BlockWrite((classNode) => {
                                                ...
                                                default:
                                                    ...
                                                    if (mType is ArrayTypeSyntax arr) {
                                                        ...
                                                        if (elementRepeating) {
                                                            ...
                                                            //deserMemberBlock.WriteLine($"var _g_bufferCache_{indexID} = ArrayPool<{eleSym.Name}>.Shared.Rent(_g_elementCount_{indexID});");
                                                            deserMemberBlock.WriteLine($"var _g_bufferCache_{indexID} = ({eleSym.Name}*)Marshal.AllocHGlobal(sizeof({eleSym.Name}) * _g_elementCount_{indexID});");
                                                            deserMemberBlock.WriteLine($"var _g_BufferIndex_{indexID} = 0;");
                                                            deserMemberBlock.Write($"while(_g_elementCount_{indexID} > 0) ");
                                                            deserMemberBlock.BlockWrite((source) => {
                                                                if (eleSym.IsValueType) {
                                                                    source.WriteLine($"_g_bufferCache_{indexID}[_g_BufferIndex_{indexID}] = default;");
                                                                    foreach (var m in externalMembers) {
                                                                        source.WriteLine($"_g_bufferCache_{indexID}[_g_BufferIndex_{indexID}].{m.memberName} = _{m.memberName};");
                                                                    }
                                                                    source.WriteLine($"_g_bufferCache_{indexID}[_g_BufferIndex_{indexID}].ReadContent(ref ptr_current);");
                                                                }
                                                                else {
                                                                    source.WriteLine($"_g_bufferCache_{indexID}[_g_BufferIndex_{indexID}] = new (ref ptr_current);");
                                                                }
                                                                source.WriteLine($"_g_elementCount_{indexID} -= _g_bufferCache_{indexID}[_g_BufferIndex_{indexID}].{nameof(IRepeatElement<int>.RepeatCount)} + 1;");
                                                                source.WriteLine($"++_g_BufferIndex_{indexID};");
                                                            });
                                                            deserMemberBlock.WriteLine();
                                                            deserMemberBlock.WriteLine($"{memberAccess} = new {eleSym.Name}[_g_BufferIndex_{indexID}];");
                                                            deserMemberBlock.WriteLine($"fixed (void* _g_copyPtr_{indexID} = {memberAccess}) "); 
                                                            deserMemberBlock.BlockWrite((source) => {
                                                                source.WriteLine($"Unsafe.CopyBlock(_g_copyPtr_{indexID}, _g_bufferCache_{indexID}, (uint)(sizeof({eleSym.Name}) * _g_BufferIndex_{indexID}));");
                                                            });
                                                            deserMemberBlock.WriteLine($"Marshal.FreeHGlobal((IntPtr)_g_bufferCache_{indexID});");
                                                            deserMemberBlock.WriteLine();
                                                            goto nextMember;
                                                        }
                                                        else {
                                ...
                                if (packet.CompressData != default) {
                                    ...
                                    //v2
                                    //writeNode.WriteLine($"var ptr_compressed = ptr_current;");
                                    //writeNode.WriteLine($"var Ptr_rawdata = Marshal.AllocHGlobal({packet.CompressData.bufferSize});");
                                    //writeNode.WriteLine("ptr_current = (void*)Ptr_rawdata;");
                                    //writeNode.WriteLine();

                                    //readNode.WriteLine($"var Ptr_decompressed = Marshal.AllocHGlobal({packet.CompressData.bufferSize});");
                                    //readNode.WriteLine("var ptr_current = (void*)Ptr_decompressed;");
                                    //readNode.WriteLine($"CommonCode.ReadDecompressedData(ptr, ref ptr_current, (int)((long)ptr_end - (long)ptr));");
                                    //readNode.WriteLine("ptr_current = (void*)Ptr_decompressed;");
                                    //readNode.WriteLine();

                                    //UnfoldMembers(writeNode, readNode, typeSym, packet.Members.Select<MemberDataAccessRound, (MemberDataAccessRound, string?)>(m => (m, null)));
                                    //writeNode.WriteLine();
                                    //writeNode.WriteLine($"CommonCode.WriteCompressedData((void*)Ptr_rawdata, ref ptr_compressed, (int)((long)ptr_current - (long)Ptr_rawdata), {packet.CompressData.compressLevel});");
                                    //writeNode.WriteLine($"Marshal.FreeHGlobal(Ptr_rawdata);");
                                    //writeNode.WriteLine("ptr = ptr_compressed;");

                                    //readNode.WriteLine();
                                    //readNode.WriteLine("ptr = ptr_end;");
                                    //readNode.WriteLine($"Marshal.FreeHGlobal(Ptr_decompressed);");

                                    //v3
                                    writeNode.WriteLine($"var ptr_compressed = ptr_current;");
                                    writeNode.WriteLine($"var memoryOwner = MemoryPool<byte>.Shared.Rent({packet.CompressData.bufferSize});");
                                    writeNode.Write("fixed (void* ptr_rawdata = memoryOwner.Memory.Span)");

                                    readNode.WriteLine($"var memoryOwner = MemoryPool<byte>.Shared.Rent({packet.CompressData.bufferSize});");
                                    readNode.Write("fixed (void* ptr_decompressed = memoryOwner.Memory.Span)");
                                    ...
                                    (writeNode, readNode).BlockWrite((writeNode, readNode) => {
                                        ...
                                        UnfoldMembers(writeNode, readNode, typeSym, packet.Members.Select<MemberDataAccessRound, (MemberDataAccessRound, string?)>(m => (m, null)));
                                        writeNode.WriteLine($"CommonCode.WriteCompressedData(ptr_rawdata, ref ptr_compressed, (int)((long)ptr_current - (long)ptr_rawdata), {packet.CompressData.compressLevel});");
                                        writeNode.WriteLine($"memoryOwner.Dispose();");
                                        writeNode.WriteLine("ptr = ptr_compressed;");
                                    });
                                    readNode.WriteLine("ptr = ptr_end;");
                                    readNode.WriteLine($"memoryOwner.Dispose();");
                                }
                                else {
